...
func previousSources(history []storage.Message) []tools.Post {
	...
	for _, message := range history {
		...
		for _, link := range sourceLink.FindAllString(message.Content, -1) {
			if tools.IsSourceURL(link) && !seen[link] {
...
func resolveCitations(text string, sources []tools.Post) string {
	...
	locations := make(map[string]string, len(sources))
	verified := make(map[string]bool, len(sources))
	for _, source := range sources {
		links[source.ID] = source.URL
		locations[source.ID] = source.Locator
		if source.URL != "" {
	...
	text = referenceLink.ReplaceAllStringFunc(text, func(link string) string {
		...
		if location := locations[parts[2]]; location != "" {
			return location
		}
		return parts[1] + "（原帖未核实）"
	})
	text = markdownSourceLink.ReplaceAllStringFunc(text, func(link string) string {
		parts := markdownSourceLink.FindStringSubmatch(link)
	...
	text = postReference.ReplaceAllStringFunc(text, func(id string) string {
		...
		if location := locations[id]; location != "" {
			return location
		}
		return "来源未核实"
	})
	return sourceLink.ReplaceAllStringFunc(text, func(link string) string {
		if verified[link] {
			return link
		}
		return "（原帖未核实）"
...
func withSourceLinks(text string, sources []tools.Post) string {
	for _, source := range sources {
		...
		if source.Locator != "" && strings.Contains(text, source.Locator) {
			return text
		}
	}
	...
	for _, source := range ordered {
		if source.URL == "" && source.Locator != "" && !seen[source.Locator] {
			seen[source.Locator] = true
			links = append(links, escape.Replace(source.Title)+"（"+source.Locator+"）")
			continue
		}
