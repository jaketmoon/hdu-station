...
type CampusSession struct {
	...
	// Product scope; raw query helpers remain usable by diagnostics.
	XiashaOnly        bool
	automaticMatching bool
	maxCourses        int
}
...
func (s *CampusSession) resolveTerm(ctx context.Context, in OfferingInput) (AcademicTerm, string, error) {
	...
	if !t.valid() || cfg.Course.SchoolYear != t.SchoolYear || string(cfg.Course.Semester) != strconv.Itoa(t.Semester) {
		return AcademicTerm{}, "", errors.New("教务开课与课表默认学期缺失或不一致，暂时无法核实本学期；请确认教务配置后重试。")
	}
	s.term = &t
...
func (s *CampusSession) offerings(ctx context.Context, in OfferingInput) (OfferingResult, error) {
	...
	for i, name := range names {
		...
		go func(i int, name string) {
			...
			q := s.searchOffering(ctx, result.Term, name, keywords[name])
			if s.automaticMatching {
				result.Queries[i] = resolveCourseNames(q)
			} else {
				result.Queries[i] = selectCourseIDs(q, in.CourseIDs)
			}
		}(i, name)
	}
	wg.Wait()
...
func (s *CampusSession) searchOffering(ctx context.Context, term AcademicTerm, name, core string) OfferingQuery {
	...
	for stage, word := range []string{name, core} {
		...
		for page := 0; page < pages; page++ {
			...
			for _, r := range data.Classes {
				...
				if s.XiashaOnly && !strings.Contains(o.Campus, "下沙") {
					if s.dictionary["campusID"][r.CampusID] == "" {
						unknownCampus = true
					}
					continue
				}
				...
				if strings.TrimSpace(r.CourseName) == name || r.CourseID == name {
	...
	if unknownCampus {
		result.Warning = strings.TrimSpace(result.Warning + " 部分开课记录的校区未能核实，未纳入下沙候选；请在教务系统确认。")
	}
	if result.Warning == "" {
		s.mu.Lock()
