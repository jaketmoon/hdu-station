...
type FitCoursesResult struct {
	...
	PlanLimited      bool            `json:"planLimited,omitempty"`
...
func (s *CampusSession) FitCourses(ctx context.Context, in FitCoursesInput) (result FitCoursesResult, err error) {
	...
	defer cancel()
	result = FitCoursesResult{CheckedAt: time.Now().Format(time.RFC3339), BusyTimes: []CourseTime{}, Fits: []CourseFit{}, Warnings: []string{"仅按教务课表核对整学期的周次、星期、节次；未校验选课资格、余量、考试冲突或跨校区通勤。fits表示单门与原课表兼容；suggestedPlan是一组相互无冲突且每个课程号最多一个班的候选组合，不是已选课；仅在已查询候选范围内寻找最多门数的组合。"}}
	defer func() {
		s.lastFit = &result
	...
	// Some timetable rows omit a comparable course number. Link their selected
	// class IDs to this same-term official catalog before evaluating any class,
	// so another class of an already selected course cannot enter the plan.
	if schedule.courses == nil {
		schedule.courses = map[string]bool{}
	}
	markSelectedCourse := func(o Offering) {
		if o.ClassID != "" && o.CourseID != "" && schedule.classes[o.ClassID] {
			schedule.courses[o.CourseID] = true
		}
	}
	for _, q := range offerings.Queries {
		for _, o := range q.Classes {
			markSelectedCourse(o)
		}
	...
	var complete bool
	result.SuggestedPlan, complete = bestCoursePlan(result.Fits, s.maxCourses)
	result.PlanLimited = !complete
