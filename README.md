...
推荐后可追问“这些课本学期开哪些班？”或“结合我的课表，空闲时间能塞哪几门课？周五不要排课”。助手只核实教务默认本学期、下沙校区；网上名称先查完整名称，再查核心词，按课程号整理候选。词干等价名称由程序匹配，过于模糊时要求补充完整名称。课表筛选按整学期周次、星期和节次计算，保留冲突与未确认状态，并给出一组彼此无冲突的候选班级。不显示班级号；下学期、其他校区、钟点换算、课程余量、选课资格、考试冲突和学分认定暂不支持，不执行真实选课；课程分类查询仍未接入。
