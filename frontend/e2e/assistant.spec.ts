...
test("course fit recommendation and course IDs remain readable", async ({
  ...
  await expect(page.getByRole("table")).not.toContainText("班级号");
  await expect(page.getByRole("table")).not.toContainText("class-a");
